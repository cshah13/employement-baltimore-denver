...
1. What does the number of jobs available per square mile look like in Baltimore and Denver?
2. What percentage of the people in each city earned a college degree?
...
#### PYTHON ANALYSIS
This same analysis was conducted on Python with modifications. The [full data analysis on Google Collab can be found here](https://github.com/cshah13/workforce-opportunities-baltimore-denver/blob/main/Shah_Baltimore_Denver_Jobs_Analysis.ipynb). The HTML graphs dervied from PLOTLY Express have been added as files to this repository, however the file sizes are too large to view. Therefore, a screenshot of both graphs for both Baltimore and Denver can be seen below. The Python graphs included additional locations in both Denver and Baltimore that were not included in the Excel analysis, increasing the maximum number of jobs available in a location in both cities. When creating the graphs in Excel, the software automatically formated the x-axis to be neat and organized, hindering all location names to be shown in the axis. Using Python required more steps to format the graph, but all of the location names are viewable. Furthermore, Python was easier to use, as altering a code provided the desired outcome rather than needing to navigate through various formulas and buttons on Excel. Overall, the graph from Python looks more  professional and detailed.
...
#### PYTHON ANALYSIS
The college degree data was further developed through a Python analysis. A bar chart was created to directly visualize and compare the differences. We can see the 10% difference clearly on the Python graph. It was easier to create a neat visualization on Python, as the software automatically creates a visually appealing output. In word, the output need to be heavily manipulated to become visually appealing. This graph is more clear and a better visualization for college degree holders in Denver and Baltimore. 
