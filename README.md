...
#### PYTHON ANALYSIS
This same analysis was conducted on Python with modifications. The [full data analysis on Google Collab can be found here](https://github.com/cshah13/workforce-opportunities-baltimore-denver/blob/main/Shah_Baltimore_Denver_Jobs_Analysis.ipynb). Both graphs for both Baltimore and Denver can be seen below. When creating the graphs in Excel, the software automatically formated the x-axis to be organized, hindering all location names to be shown in the axis. Using Python required more steps to format the graph, but all of the location names are viewable. Furthermore, Python was easier to use, as altering a code provided the desired outcome rather than needing to navigate through various formulas and buttons on Excel. Although it was a timely process to accurately manipulate the code, the graph from Python looks more professional and detailed.
