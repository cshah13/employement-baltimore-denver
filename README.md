...
#### PYTHON ANALYSIS
This same analysis was conducted on Python with modifications. The [full data analysis on Google Collab can be found here](https://github.com/cshah13/workforce-opportunities-baltimore-denver/blob/main/Shah_Baltimore_Denver_Jobs_Analysis.ipynb). The HTML graphs dervied from PLOTLY Express have been added as files to this repository, however the file sizes are too large to view. Therefore, a still graph for both Baltimore and Denver can be seen below. The graphs included additional locations in both Denver Baltimore that were not included in the Excel analysis, altering the maximum number of jobs available in a specific location in Baltimore. When creating the graphs in Excel, the software automatically formated the axis to be neat and organized, hindering all location names to be shown in the axis. Using Python required more steps to format the graph, but all of the location names are viewable. Furthermore, Python was easier to use, as altering a code provided the desired outcome rather than needing to navigate through various formulas and buttons on Excel.
