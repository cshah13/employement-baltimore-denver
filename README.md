...
#### PYTHON ADDITIONS 
The Python analysis led to the same findings since the data was the same. Denver has a greater number of jobs available per square mile along with more people with college degrees. This information is useful for governments and nonprofit organizations. The organizations can further analyze the underlying differences between the two cities to understand why the gaps exist. For example, if higher education is not promoted as much in Baltimore Public Schools, the government can focus more attention on higher education preparation to increase the proportion of the population with college degrees. In the Python analysis, the graphs are more visually appealing compared to the graphs from Excel. In the Python analysis, I extended the analysis by adding a graph to show the proportion of the population who earned a college degree in Baltimore and Denver. Although the findings are the same, the Python graph allows the audience the better visualize the findings. By using a graph as opposed to a table, we can see the direct differences in the proportion of college degree holders between the two cities. This new graph should be presented to government and non profit organizations to efficiently show the gap in college degree holders between Baltimore and Denver. Although Denver has a higher amount of college degree holders and jobs available compared to Baltimore, there is still work that needs to be done to increase the rates in Denver. To build on these findings, additional data such as government assistance and investement in education can be analyzed to identify the underlying causes behind the differences in workforce opportunities in Baltimore and Denver. Based on the findings, programs can be implemented to increase the availability of jobs and number of people with college degrees in each city.
